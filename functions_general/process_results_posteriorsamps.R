...
process_results <- function(df_full, tmbdat, samps1, polyOrder, 
                            AR = FALSE,single_station = TRUE){
  
  P = as.matrix(tmbdat$P)
  X = as.matrix(tmbdat$X)
  if (single_station==FALSE){
  ...
  vderiv_full <- compute_post_fun(samps = coefsamps1, global_samps = global_samps1, 
                             knots = knots, 
                             refined_x = df_full$t,
                             p = polyOrder, degree = 1)
  ...
  v_u_fixed_deriv <-vderiv_full+ u_deriv_samps1}
  
  v <- v %>% 
    mutate(sample_date = unique(df_full$sample_date))
  ...
  v_fixed <- v_fixed %>% 
    ...
    melt(id.vars = c("sample_date", "site_id"))}else{v_fixed = NULL}
  ...
  if (AR == TRUE){
    ...
    v_u_fixed_deriv <- v_u_fixed_deriv %>% 
      mutate(sample_date = df_full$sample_date, 
             site_id = df_full$site_id) %>% 
      melt(id.vars = c("sample_date", "site_id"))
  } else {v_u = NULL; v_u_fixed = NULL; v_u_fixed_deriv = NULL}
  
  
  return(list(v = v,vderiv=vderiv, v_fixed=v_fixed, v_u = v_u, v_u_fixed = v_u_fixed, v_u_fixed_deriv=v_u_fixed_deriv))
}