...
process_results <- function(df_full, tmbdat, samps1, polyOrder, AR = FALSE,version =1, out = out ){
  
  df_full <- df_full %>% 
    mutate(train = TRUE)
  
  stations <- df_full$site_id %>% unique()
  sample_date <- c(unique(df_full$sample_date), max(unique(df_full$sample_date))+ 1:out)
  
  df_full <- expand.grid(sample_date = sample_date, 
              site_id = stations) %>% 
    mutate(t = as.numeric(sample_date - min(sample_date))) %>% 
    left_join(df_full, by = c("t","sample_date","site_id")) %>% 
    mutate(train = ifelse(is.na(train), FALSE, train)) 
  ...
  v <- v %>% 
    mutate(sample_date = df_full$sample_date[1:nrow(v)],
           train = df_full$train[1:nrow(v)])
  
  v <- v %>% 
    melt(id.vars = c("sample_date","train"))
  
  v_fixed <- v_fixed %>% 
    mutate(sample_date = df_full$sample_date, 
           site_id = df_full$site_id,
           train = df_full$train) %>% 
    melt(id.vars = c("sample_date", "site_id","train"))
