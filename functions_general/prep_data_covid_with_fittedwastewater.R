...
if (!is.null(add_lag)){
  results <- lapply(results, function(x){if(!is.null(x)){x$sample_date = x$sample_date + days(7*add_lag)}; return(x)})
}
...
tmp_ar <- lapply(list(results$v_u, results$v_u_fixed,results$v_fixed), 
       function(x){
         tmp2 <- x %>%
           ...
           left_join(start_dates, by = "site_id") %>% 
           mutate(inc = sample_date >= date_inc) %>% 
           left_join(weights_datadic, by = "site_id") %>% 
           mutate(weight = ifelse(inc, weight, 0)) %>% 
           mutate(value = value*weight) %>%
           group_by(variable, sample_date) %>%
           summarise(value = sum(value)/sum(weight)) %>%
